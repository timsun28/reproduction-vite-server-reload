...
import type {
    CreateMethodPipeline,
    CreatePubPipeline,
    PipelineContext,
    SubscriptionCallbacks,
} from "./types";
import {
    createReactiveCursorPublisher,
    flattenPipeline,
    isThenable,
    partialPipeline,
    Subscribe,
...
export function createMethod<S extends z.ZodTypeAny, T>(config: {
    name?: string;
    schema: S;
    ...
    ...args: S extends z.ZodUndefined ? [] : [z.input<S>]
) => Promise<Awaited<T>>;
export function createMethod<S extends z.ZodTypeAny>(config: {
    name?: string;
    schema: S;
    rateLimit?: { interval: number; limit: number };
...
>(config: {
    ...
    if (!config.name) {
        throw new Error(
            `Method name is missing. Do you have the relay babel plugin configured?`,
        );
    }
    ...
    Meteor.methods({
        async [name](data) {
            if (pipeline.length === 0) {
                throw new Error(
                    `Pipeline or run function for ${name} never configured`,
                );
            }
    ...
    function call(args?: z.input<S>): Promise<Awaited<T>> {
        return new Promise((resolve, reject) => {
            Meteor.call(
                name,
                args,
                (err: null | Meteor.Error, result: Awaited<T>) => {
                    if (err) {
                        reject(err);
                    } else {
                        resolve(result);
...
export function createPublication<S extends z.ZodTypeAny, T>(config: {
    ...
    ...args: z.output<S> extends undefined
        ? [SubscriptionCallbacks?]
        : [z.output<S>, SubscriptionCallbacks?]
) => Meteor.SubscriptionHandle;
export function createPublication<S extends z.ZodTypeAny, R1>(config: {
    name?: string | null;
    schema: S;
    rateLimit?: { interval: number; limit: number };
}): { pipeline: CreatePubPipeline<z.output<S>> };
export function createPublication<S extends z.ZodTypeAny, T>(config: {
    name?: string | null;
    schema: S;
    rateLimit?: { interval: number; limit: number };
    ...
    if (typeof config.name === "undefined") {
        throw new Error(
            `Publication name is missing. Do you have the relay babel plugin configured?`,
        );
    }
    ...
    Meteor.publish(name, async function (data: z.input<S>) {
        if (pipeline.length === 0) {
            throw new Error(
                `Pipeline or run function never configured for ${name} publication`,
            );
        }
        ...
        async function run(startIndex: number, startInput: any) {
            if (isRunning) {
                console.log(
                    "zodern:relay Tried to run pipeline while it is already running??",
                );
                return;
            }
            ...
            for (let index = startIndex; index < fullPipeline.length; index++) {
                ...
                if (input && input[Subscribe]) {
                    let stopSubscription = input[Subscribe](
                        (newResult: any) => {
            ...
            if (stopPipelineSubscriptions.length > 0) {
                ...
                if (output && !Array.isArray(output) && output._publishCursor) {
                    ...
                    output.every((c) => c._publishCursor)
                ) {
                    reactivePublisher.updateCursors(output);
                }
            } else {
    ...
    function subscribe(
        ...args: S extends z.ZodUndefined
            ? [SubscriptionCallbacks?]
            : [z.input<S>, SubscriptionCallbacks?]
    ): Meteor.SubscriptionHandle {
        if (name === null) {
